...
class gameWindow(pyglet.window.Window):
    def __init__(self, *args, **kwargs):
        ...
        global Board, badBatch
        global y_min, y_max, x0_min, x0_max, x1_min, x1_max, x2_min, x2_max, x3_min, x3_max, x4_min, x4_max
        ...
        self.set_icon(blackCursorImage)

        self.blackCursor = pyglet.window.ImageMouseCursor(blackCursorImage, 11, 11)
        self.brownCursor = pyglet.window.ImageMouseCursor(brownCursorImage, 11, 11)
        self.redCursor = pyglet.window.ImageMouseCursor(redCursorImage, 11, 11)
        self.orangeCursor = pyglet.window.ImageMouseCursor(orangeCursorImage, 11, 11)
        self.yellowCursor = pyglet.window.ImageMouseCursor(yellowCursorImage, 11, 11)
        self.greenCursor = pyglet.window.ImageMouseCursor(greenCursorImage, 11, 11)
        self.blueCursor = pyglet.window.ImageMouseCursor(blueCursorImage, 11, 11)
        self.grayCursor = pyglet.window.ImageMouseCursor(grayCursorImage, 11, 11)
        self.whiteCursor = pyglet.window.ImageMouseCursor(whiteCursorImage, 11, 11)
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if (y_min < y < y_max) and Board.correct == False:
            ...
            if (x4_min < x < x4_max) and Board.superMM == True: 
...
class board:
    def __init__(self, mode, supermm):
        ...
        if supermm:
            ...
            self.currentGuess = [BACKGROUND_COLOR, BACKGROUND_COLOR, BACKGROUND_COLOR, BACKGROUND_COLOR, BACKGROUND_COLOR]
            ...
            self.currentGuess = [BACKGROUND_COLOR, BACKGROUND_COLOR, BACKGROUND_COLOR, BACKGROUND_COLOR]
    ...
    def checkGuess(self):
        self.lastGuess = self.currentGuess
        
        for PIN in range(self.pinNumber - 1, -1, -1):
            if self.solution[PIN] in self.currentGuess:
                if self.solution[PIN] == self.currentGuess[PIN]:
                    self.currentFeedback[PIN] = BLACK
                else:
                    self.currentFeedback[PIN] = WHITE
                self.currentGuess.remove(self.solution[PIN])
            else:
        ...
        if self.lastGuess == self.solution or self.activeRow == 11:
    ...
    def drawBoard(self):
        ...
        self.title = pyglet.text.Label('Master Mind', font_name = FONT, font_size=30, 
            ...
            batch = badBatch, group = window.layer3)
        ...
        self.modeText = pyglet.text.Label(sStr, font_name = FONT, font_size = 25, 
            x = SCREEN_WIDTH/2 + 40, y = 16*GRIDSIZE, batch = badBatch, group = window.layer1)
        ...
        for row in range(12):
            self.numbers.append(pyglet.text.Label(str(row + 1), font_name = FONT, font_size = 14, 
                ...
                batch = badBatch, group = window.layer1))
    ...
    def changeColor(self, dir):
        ...
        if self.currentColor == BLACK: window.set_mouse_cursor(window.blackCursor)
        if self.currentColor == BROWN: window.set_mouse_cursor(window.brownCursor)
        if self.currentColor == RED: window.set_mouse_cursor(window.Cursor)
        if self.currentColor == ORANGE: window.set_mouse_cursor(window.orangeCursor)
        if self.currentColor == YELLOW: window.set_mouse_cursor(window.yellowCursor)
        if self.currentColor == GREEN: window.set_mouse_cursor(window.greenCursor)
        if self.currentColor == BLUE: window.set_mouse_cursor(window.blueCursor)
        if self.currentColor == BACKGROUND_COLOR: window.set_mouse_cursor(window.grayCursor)
        if self.currentColor == WHITE: window.set_mouse_cursor(window.whiteCursor)
...
class pin:
    def __init__(self, pinColor, row, col, rad, fb = False):
        window = super()
        if not fb:
            window.circle = shapes.Circle(col * GRIDSIZE + GRIDSIZE, GRIDSIZE + row * GRIDSIZE , rad, color = pinColor, batch = badBatch, group = window.layer1)
        else: 
            window.circle = shapes.Circle((7.5+col)*GRIDSIZE, (1+row)*GRIDSIZE, rad, color = pinColor, batch = badBatch, group = window.layer1)




window = gameWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Master Mind", resizable=True)

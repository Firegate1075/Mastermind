...
layer0 = pyglet.graphics.Group(0)
layer1 = pyglet.graphics.Group(1)
layer2 = pyglet.graphics.Group(2)
layer3 = pyglet.graphics.Group(3)
...
window.set_icon(redCursorImage)
...
class board:
    def __init__(self, mode, supermm):
        ...
        if supermm:
            ...
            self.currentFeedback = [NONE_COLOR, NONE_COLOR, NONE_COLOR, NONE_COLOR, NONE_COLOR]
            self.pinNumber = 5
        else:
            ...
            self.currentFeedback = [NONE_COLOR, NONE_COLOR, NONE_COLOR, NONE_COLOR]
            self.pinNumber = 4
    ...
    def checkGuess(self):
        self.lastGuess = self.currentGuess.copy()
        self.currentSolution = self.solution.copy()

        for PIN in range(self.pinNumber):
            if self.currentSolution[PIN] == self.currentGuess[PIN]:
                self.currentFeedback[PIN] = BLACK
                self.currentGuess[PIN] = NONE_COLOR
                self.currentSolution[PIN] = NONE_COLOR_2

        for PIN_SOLUTION in range(self.pinNumber):
            for PIN_GUESS in range(self.pinNumber):
                if self.currentSolution[PIN_SOLUTION] == self.currentGuess[PIN_GUESS]:
                    
                    self.currentFeedback[PIN_SOLUTION] = WHITE
                    self.currentGuess[PIN_GUESS] = NONE_COLOR


            if self.currentFeedback[PIN_SOLUTION] == NONE_COLOR:
                self.currentFeedback[PIN_SOLUTION] = BACKGROUND_COLOR
...
def on_mouse_press(x, y, button, modifiers):
    ...
    if button == pyglet.window.mouse.RIGHT:
        if (y_min < y < y_max) and Board.correct == False:
            if (x0_min < x < x0_max): 
                Board.pins.append(pin(BACKGROUND_COLOR, Board.activeRow, 0, PIN_SIZE))
                Board.currentGuess[0] = BACKGROUND_COLOR
            if (x1_min < x < x1_max): 
                Board.pins.append(pin(BACKGROUND_COLOR, Board.activeRow, 1, PIN_SIZE))
                Board.currentGuess[1] = BACKGROUND_COLOR
            if (x2_min < x < x2_max): 
                Board.pins.append(pin(BACKGROUND_COLOR, Board.activeRow, 2, PIN_SIZE))
                Board.currentGuess[2] = BACKGROUND_COLOR
            if (x3_min < x < x3_max): 
                Board.pins.append(pin(BACKGROUND_COLOR, Board.activeRow, 3, PIN_SIZE))
                Board.currentGuess[3] = BACKGROUND_COLOR
            if (x4_min < x < x4_max) and Board.superMM == True: 
                Board.pins.append(pin(BACKGROUND_COLOR, Board.activeRow, 4, PIN_SIZE))
                Board.currentGuess[4] = BACKGROUND_COLOR
